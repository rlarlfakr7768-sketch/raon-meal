# 급식 → 인스타 자동 게시 (클라우드 / PC 꺼도 됨)

> **2026-08-16 급식 게시는 잠정 중단.** 예약(cron)을 끄고 수동 실행만 남겼고,
> 로컬 작업 스케줄러도 사용 안 함으로 돌렸다. 계정은 phyedu 콘텐츠(지식, 과학뉴스,
> IT 트렌드)만 올린다. 다시 켜는 방법은 daily.yml, weekly.yml 맨 위 주석에 적어 뒀다.
...
### 4) 테스트 실행
- 레포 → **Actions** 탭 → "급식 자동 게시 (중단됨)" → **Run workflow** (수동)
